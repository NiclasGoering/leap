...
def generate_architecture(input_dim, depth, k_factor):
    """
    Generate a neural network architecture based on the input dimension, depth, and k-factor.
    
    Args:
        input_dim: Input dimension (d)
        depth: Number of hidden layers
        k_factor: Growth factor for layer widths
    ...
    for i in range(depth):
        # Apply k-factor and round to nearest integer
        current_width = int(round(current_width * k_factor))
        architecture.append(current_width)
...
def main():
    ...
    input_dimensions = [8,16, 32, 64, 128]
    
    # List of network depths to explore
    depths = [4]
    
    # List of k-factors to explore
    k_factors = [2/3, 1.0, 1.5]
    ...
    data_base_dir = "/scratch/goring/NN_data/NN_0304_k"
    ...
    for input_dim in input_dimensions:
        for depth in depths:
            for k_factor in k_factors:
                # Generate architecture based on k-factor
                architecture = generate_architecture(input_dim, depth, k_factor)
                for variance in init_variances:
                    for exp_num in range(1, num_experiments + 1):
                        total_combinations.append((input_dim, depth, k_factor, architecture, variance, exp_num))
    ...
    for idx, (input_dim, depth, k_factor, architecture, variance, exp_num) in enumerate(my_combinations):
        # Create a compact name for this combination
        arch_str = format_architecture(architecture)
        var_str = f"var{variance}".replace(".", "p")
        k_str = f"k{k_factor}".replace(".", "p")
        run_name = f"NN_gaussian_d{input_dim}_depth{depth}_{k_str}_arch{arch_str}_{var_str}"
        
        print(f"\nProcess {rank} starting combination {idx+1}/{len(my_combinations)}: {run_name} (Exp {exp_num})")
        print(f"Architecture generated with k-factor {k_factor}: {architecture}")
        ...
        try:
            ...
            smart_name = f"NN_gaussian_d{input_dim}_depth{depth}_{k_str}_arch{arch_str}_{var_str}_{exp_num}"
            ...
            try:
                ...
                results = {
                    'hyperparameters': {
                        ...
                        'experiment_description': f"Random NN #{exp_num} with k-factor {k_factor}",
