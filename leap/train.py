...
def extract_info_from_msp_path(path):
    """Extract parameters from MSP dataset path"""
    basename = os.path.basename(path)
    info = {"distribution_type": "MSP"}  # Mark as MSP type
    ...
    # Extract degree
    deg_match = re.search(r'deg(\d+)', basename)
    if deg_match:
        info['order_num'] = deg_match.group(1)
        
    # Extract leap
    leap_match = re.search(r'leap(\d+)', basename)
    if leap_match:
        info['leap'] = int(leap_match.group(1))
...
def generate_unique_id(config):
    ...
    unique_id = (
        f"{ds_name}_O{order_num}_leap{leap}"
...
def find_msp_dataset_files(directory):
    """Find MSP dataset X and y files in the given directory"""
    try:
        ...
        # Find MSP dataset files
        x_files = [f for f in all_files if 'dataset_X_MSP_' in os.path.basename(f)]
        y_files = [f for f in all_files if 'dataset_y_MSP_' in os.path.basename(f)]
        
        if not x_files or not y_files:
            print(f"No MSP dataset files found in {directory}")
            return None
        ...
        for x_file in x_files:
            ...
            for y_file in y_files:
                ...
                if x_name == y_name:
                    print(f"Found matching MSP dataset X and y files")
        ...
        print(f"Error finding MSP dataset files: {str(e)}")
        return None

def load_dataset_info(directory):
    """Load MSP dataset info"""
    if not os.path.isdir(directory):
        return None
    
    # Find dataset files
    dataset_files = find_msp_dataset_files(directory)
    if not dataset_files:
        return None
    
    # Extract parameters
    params = extract_info_from_msp_path(directory)
...
def load_dataset_directly(dataset_files, device):
    """Load MSP dataset directly to GPU"""
    data = {'X': None, 'y': None}
...
def generate_all_combinations(config):
    ...
    for sweep_name, sweep_info in sweeps.items():
        ...
        print(f"Loaded {len(dataset_infos)} valid MSP datasets for sweep {sweep_name}")
        ...
        for ds_info in dataset_infos:
            ...
            # Pass the order number and leap to the combinations
            order_num = ds_params.get('order_num')
            leap = ds_params.get('leap', 0)
                                ...
                                for alignment in sweep_params.get("alignment", [False]):
                                    for exp_num in range(1, base_cfg.get("num_experiments", 1) + 1):
                                        combo = {
                                            ...
                                            'leap': leap,  # Add leap parameter
                                            'size_mb': ds_info.get('size_mb', 0),
                                            'distribution_type': 'MSP',  # Always MSP
...
def fast_parallel_training(config_batch, device, X_full, y_full, base_config, 
    ...
    for i in range(len(models)):
        ...
        leap = config_item.get('leap', 0)
        order_num = config_item.get('order_num', "1")
        ...
        result = {
            ...
            'mode': config_item['mode'],
            'alignment': config_item['alignment'],
            'distribution_type': 'MSP',
            'leap': leap,  # Include leap parameter
            'order_num': order_num,  # Include order number
